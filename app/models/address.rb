class Address < ApplicationRecord
  ...
  validates :kana_first_name, presence: { message: "名カナ を入力してください" },  on: :address_create, length: { maximum: 35, message:"35文字以下で入力してください" }, format: { with: VALID_KATAKANA_REGIX, message: "カナ文字で入力してください"}
  validates :postal_code,     presence: { message: "郵便番号 を入力してください" }, on: :address_create, format: { with: VALID_POSTAL_CODE_REGIX, message: "フォーマットが不適切です" } 
  ...
  validates :tel,                                                              on: :address_create, format: { with: VALID_TEL_REGIX, message:"電話番号 を入力してください", allow_blank: true }
