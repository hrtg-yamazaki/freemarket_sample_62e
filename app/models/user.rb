class User < ApplicationRecord
  ...
  validates :password,       presence: { message: "パスワード を入力してください" },     on: :registration_post, length: { maximum: 128, message: "128文字以下で入力してください" }, format: { with: VALID_PASSWORD_REGIX, message: "英字と数字両方を含むパスワードを入力してください" }
  ...
  validates :kana_first_name, presence: { message: "名カナ を入力してください" },        on: :registration_post, length: { maximum: 35, message: "35文字以下で入力してください" }, format: { with: VALID_KATAKANA_REGIX, message: "名カナは カナ文字を入力してください" }
